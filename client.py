...
class Client:
    ...
    def execute_server_generated_commands(self,msg):  # one star near the message
        command = msg["command"]
        note = msg["note"]
        if note:
            print(note.upper())
        if command=="-login_accept:":
            self.logged_in = True
            self.change_message_property("from",msg["to"])
    ...
    def update_message_state(self, msg):
        ...
        if self.to_send == True:  # if send flag is true
            ...
            formatted_msg_len = parser.format_message_length(len(formatted_message))
